...
diretorio_atual = os.path.abspath(os.path.dirname(__file__))# Obter o caminho absoluto do diretório em que o código está sendo executado
...
wb = xw.Book("cali.xlsx")# abre a planilha
ws = wb.sheets['cali']
...
#----------------------------------------------
#------------------------------------------------------------------------------------------------------
#impotar as colunas da planilha de armazenamnto ness as variavies: 
...
def co_57(valor_procurado, ekev, resolucao, contagem, incerteza, canal):
    ...
    for i in range(len(canal)):
        for j in range(len(ekev)):
            if abs(float(ekev[j].replace(',', '.')) - valor_procurado) <= 1:
                encontrado = True
                print("O valor é: ", j)
...
co_57(valor_procurado, ekev, resolucao, contagem, incerteza, canal)
#----------------------------------------------------------------------------------------------------------------

#-----------------------------------------COBALTO 60-------------------------------------------------------------
valor_procurado= 1332.5
def co_60(valor_procurado, ekev, resolucao, contagem, incerteza, canal):
    global co_60_ekev, co_60_resolucao, co_60_contagem, co_60_incerteza, co_60_canal
    encontrado = False
    for i in range(len(canal)):
        for j in range(len(ekev)):
            if abs(float(ekev[j].replace(',', '.')) - valor_procurado) <= 1:
...
co_60(valor_procurado, ekev, resolucao, contagem, incerteza, canal)
#----------------------------------------------------------------------------------------------------------------
...
ws2.range('H9').options(transpose=True).value = [co_60_contagem]
ws2.range('L9').options(transpose=True).value = [co_60_incerteza]
ws2.range('M9').options(transpose=True).value = [usuario]

print(co_60_ekev)
print(co_60_resolucao)
print(co_60_canal)
print(co_60_contagem)
print(co_60_incerteza)
print(usuario)
